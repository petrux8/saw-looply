  ...
  return (
    <ul className="list-group">
      {habitsOfDay.map((habit) => (
        <li
          key={habit.id}
          className="list-group-item mb-2 bg-light rounded border-0"
        >
          <div className="d-flex align-items-center gap-3">
            <div className="d-flex align-items-center">
              <input
                type="checkbox"
                className={`form-check-input ${isFuture ? "border-transparent" : "border-primary"}`}
                checked={historyOfDay ? historyOfDay[habit.id] == true : false}
                onChange={(event) =>
