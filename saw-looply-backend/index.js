...
async function checkAndSendScheduledNotifications() {
  ...
  try {
    // Qui puoi cambiare la query secondo la tua struttura DB
    // Per esempio, prendi tutti gli utenti con notifiche da mandare (esempio semplificato)
    ...
    usersSnapshot.forEach(async (userDoc) => {
      ...
      //ottengo tutti gli habit dell'user
      //filtro gli habit in base a se sono notificabili e a che ora
      const q1 = habitsRef.where("freq", "==", "daily");
      const q2 = habitsRef.where("days", "array-contains", weekDay);
